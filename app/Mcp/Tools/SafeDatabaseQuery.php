...
#[IsReadOnly]
class SafeDatabaseQuery extends Tool
{
    /**
     * The tool's description.
     */
    protected string $description = 'Execute a read-only SQL query against non-sensitive business data tables only.';
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'query' => $schema->string()
                ->description('The SQL query to execute. Only read-only queries are allowed (i.e. SELECT, SHOW, EXPLAIN, DESCRIBE). Access to sensitive tables is blocked.')
                ->required(),
    ...
    public function handle(Request $request): Response
    {
        ...
        // Security Check: Ensure no sensitive tables or keywords are used.
        foreach ($this->blockedKeywords as $blocked) {
            if (preg_match('/\b'.preg_quote($blocked, '/').'\b/i', $query)) {
                return Response::error("Query rejected: Access to the table or keyword '{$blocked}' is strictly prohibited.");
            }
        }
