<?php

declare(strict_types=1);

return [
    'auth' => [
        'login' => [
            'title' => 'Welcome back',
            'subheading' => 'Sign in to manage your FreshLeaf',
            'phone' => 'Phone Number',
            'password' => 'Password',
            'failed' => 'These credentials do not match our records.',
            'pending' => 'Your account is pending approval. Please wait for an administrator to review your application.',
            'submit' => 'Sign In',
            'remember' => 'Remember Me',
            'not_having_account' => "Don't have an account?",
            'register_here' => 'Register here',
        ],
        'register' => [
            'title' => 'Vendor Registration',
            'subheading' => 'Complete your business profile to start selling.',
            'first_name' => 'First Name',
            'last_name' => 'Last Name',
            'email' => 'Email Address',
            'password_confirm' => 'Confirm Password',
            'business_name' => 'Business Name',
            'id_front' => 'ID Card (Front)',
            'id_back' => 'ID Card (Back)',
            'store_photo' => 'Farm / Store Photo',
            'organic_cert' => 'Organic Certificate',
            'bank_name' => 'Bank Name',
            'account_holder' => 'Account Holder Name',
            'account_number' => 'Account Number',
            'qr_code' => 'Bank QR Code',
            'complete' => 'Complete Registration',
            'already_have_account' => 'Already have an account?',
            'login_here' => 'Login here',
            'phone_registered' => 'This phone number is already registered.',
            'steps' => [
                'account' => 'Account',
                'account_desc' => 'Basic information for login.',
                'business' => 'Business',
                'business_desc' => 'Information about your store or farm.',
                'verification' => 'Verification',
                'verification_desc' => 'Required identity documents.',
                'financials' => 'Financials',
                'financials_desc' => 'Payout account details.',
            ],
            'pending_title' => 'Registration Submitted',
            'pending_message' => 'Your vendor registration has been submitted. An administrator will review your application and you will be notified once approved.',
            'notification_title' => 'Vendor Registration',
            'notification_body' => 'New vendor has been registered. Please check the submission.',
            'view_vendor' => 'View Vendor',
        ],
    ],
    'product' => [
        'label' => 'Product',
        'name_en' => 'Name (English)',
        'name_km' => 'Name (Khmer)',
        'description_en' => 'Description (English)',
        'description_km' => 'Description (Khmer)',
        'system_category' => 'Product Category',
        'organic_category' => 'Organic Category',
        'unit' => 'Default Unit',
        'price' => 'Price per Unit',
        'stock' => 'Available Stock',
        'harvest_date' => 'Harvest Date',
        'general_info' => 'General Information',
        'organic_traceability' => 'Organic Traceability',
        'nutrition_data' => 'Nutrition & Data',
        'add_nutrition' => 'Add Nutrition Fact',
        'product_inventory' => 'Product Inventory',
        'quantity' => 'Quantity',
        'view_detail' => 'View Detail',
        'add_to_store' => 'Add to Store',
        'adjust_stock' => 'Adjust Stock',
        'adjustment_type' => 'Adjustment Type',
        'adjustment_type_in' => 'Stock In',
        'adjustment_type_out' => 'Stock Out',
        'adjustment_type_loss' => 'Loss',
        'adjustment_type_correction' => 'Correction',
        'quantity_change' => 'Quantity Change',
        'stock_reduction_hint' => 'Enter a negative number to reduce stock (e.g. -5)',
        'proof_photo' => 'Proof Photo',
        'reason' => 'Reason',
        'adjustment_reason_placeholder' => 'e.g. Damaged during handling, Correction after audit',
        'visuals' => 'Visuals',
        'image' => 'Image',
        'default_unit' => 'Default Unit',
        'unit' => 'Unit',
        'unit_price' => 'Unit Price',
        'currency' => 'Currency',
        'province_of_origin' => 'Province of Origin',
        'certification_type' => 'Certification Type',
        'farm_location' => 'Farm Name / Location',
        'shelf_life' => 'Shelf Life',
        'days' => 'days',
        'packaging_type' => 'Packaging Type',
        'categorization' => 'Categorization',
        'traceability' => 'Traceability',
        'pricing_inventory' => 'Pricing & Inventory',
        'status' => 'Status',
        'add_discount' => 'Add Discount',
        'discount_label' => ':percentage% OFF (:price)',
        'discount_starts_at' => 'Discount Starts At',
        'discount_ends_at' => 'Discount Ends At',
        'notifications' => [
            'added_to_store' => 'Product added to your store inventory.',
            'already_in_store' => 'This product is already in your store inventory.',
            'stock_adjusted' => 'Stock quantity adjusted successfully.',
            'discount_added' => 'Discount has been added successfully.',
        ],
    ],
    'order' => [
        'label' => 'Order',
        'plural_label' => 'Orders',
        'overview' => 'Order Overview',
        'financials' => 'Financials',
        'order_number' => 'Order Number',
        'customer' => 'Customer',
        'type' => 'Order Type',
        'status' => 'Order Status',
        'payment_status' => 'Payment Status',
        'delivery_info' => 'Delivery Information',
        'delivery_address' => 'Delivery Address',
        'delivery_date' => 'Delivery Date',
        'delivery_slot' => 'Delivery Slot',
        'notes' => 'Notes',
        'subtotal' => 'Subtotal',
        'discount' => 'Discount',
        'delivery_fee' => 'Delivery Fee',
        'tax' => 'Tax',
        'total' => 'Total Amount',
        'commission' => 'Commission',
        'product' => 'Product',
        'total_placeholder' => '0.00',
    ],
    'payout' => [
        'label' => 'Payout',
        'plural_label' => 'Payouts',
        'vendor' => 'Vendor',
        'business' => 'Business',
        'status' => 'Status',
        'method' => 'Method',
        'amount' => 'Amount',
        'currency' => 'Currency',
        'new_payout' => 'New Payout',
        'notes' => 'Notes',
        'insufficient_balance' => 'Insufficient balance.',
        'withdrawal_description' => 'Withdrawal request #:number',
        'transaction_ref' => 'Transaction Ref #',
        'processed_date' => 'Processed Date',
        'processed_at' => 'Processed At',
        'processed_by' => 'Processed By',
        'admin_notes' => 'Admin Notes',
        'details' => 'Payout Details',
        'reference' => 'Reference',
        'paid_on' => 'Paid On',
        'financial_details' => 'Financial Details',
        'bank_qr_code' => 'Bank QR Code',
    ],
    'wallet' => [
        'request_withdrawal' => 'Request Withdrawal',
        'withdrawal_amount' => 'Withdrawal Amount',
        'payout_method' => 'Payout Method',
        'withdrawal_requested' => 'Withdrawal request submitted successfully.',
        'withdrawal_description' => 'Withdrawal request #:number',
        'withdrawal_notification_title' => 'New Withdrawal Request',
        'withdrawal_notification_body' => 'Vendor #:vendor has requested a withdrawal of :amount.',
        'insufficient_balance' => 'Insufficient wallet balance.',
        'invalid_amount' => 'Invalid withdrawal amount.',
    ],
    'user' => [
        'full_name' => 'Full Name',
        'email' => 'Email',
        'phone' => 'Phone Number',
    ],
    'form' => [
        'fields' => [
            'contact_phone' => 'Contact Phone',
            'village' => 'Village',
            'commune' => 'Commune',
            'district' => 'District',
            'province' => 'Province',
            'address' => 'Address',
        ],
        'add_product' => 'Add Product',
        'save_changes' => 'Save Changes',
    ],
    'vendor' => [
        'business_name' => 'Business name',
    ],
    'profile' => [
        'save_changes' => 'Save Changes',
    ],
    'ai' => [
        'assistant' => 'AI Assistant',
    ],
    'created_at' => 'Created At',
    'updated_at' => 'Updated At',
    'timestamps' => 'Time Stamps',
    'widgets' => [
        'usd_to_khr_label' => 'USD → KHR',
        'khr_to_usd_label' => 'KHR → USD',
        'usd_to_khr' => ':rate KHR',
        'khr_to_usd' => ':rate USD',
        'usd_to_khr_desc' => '1 USD in KHR',
        'khr_to_usd_desc' => '1 KHR in USD',
    ],
    'lightbox' => [
        'zoom_out' => 'Zoom Out',
        'zoom_in' => 'Zoom In',
        'reset_zoom' => 'Reset Zoom',
    ],
    'mcp' => [
        'authorize_description' => 'This application will be able to:<br/>Use available MCP functionality.',
        'logged_in_as' => 'Logged in as:',
        'permissions' => 'Permissions:',
        'cancel' => 'Cancel',
        'authorize' => 'Authorize',
        'authorizing' => 'Authorizing...',
    ],
    'rating' => [
        'label' => 'Rating',
        'plural_label' => 'Ratings',
        'user' => 'User',
        'product' => 'Product',
        'rating' => 'Rating',
        'review' => 'Review',
        'order_item' => 'Order Item',
        'order_number' => 'Order Number',
        'created_at' => 'Rated At',
        'user_info' => 'User Information',
        'product_info' => 'Product Information',
        'rating_info' => 'Rating Information',
        'system_info' => 'System Information',
    ],
    'general' => [
        'close' => 'Close',
    ],
];
